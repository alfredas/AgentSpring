...
public class SourceDAO {
    ...
    public int saveSource(Source source) {
        ...
        if (id == null) {
            ...
            String sql = "INSERT INTO " + Table.SOURCES
                    + " (id, title, start, script) VALUES (?, ?, ?, ?);";
            final Object[] args = new Object[] { id, title, source.getStart(),
                    source.getScript() };
            this.jdbcTemplate.update(sql, args);
            return this.jdbcTemplate.queryForInt("CALL IDENTITY();");
        } else {
            // update data source
            String sql = "UPDATE " + Table.SOURCES
                    + " SET title = ?, start = ?, script = ? WHERE id = ?";
            final Object[] args = new Object[] { source.getTitle(),
                    source.getStart(), source.getScript(), source.getId() };
            this.jdbcTemplate.update(sql, args);
        }
        return id;
    }
