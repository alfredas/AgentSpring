...
public class VisualServiceImpl implements VisualService {
    ...
    private class FullVisualRowMapper implements RowMapper<Visual> {
        public FullVisualRowMapper() {
            VisualServiceImpl.this.selectedVisuals = new HashMap<Integer, Visual>();
        }

        @Override
        public Visual mapRow(ResultSet rs, int rowNum) throws SQLException {
            HashMap<Integer, Visual> map = VisualServiceImpl.this.selectedVisuals;
            ...
            if (map.get(id) == null) {
                if (clazz.equals(ChartVisual.clazz)) {
                    ...
                    throw new RuntimeException("Visual type '" + clazz + "' is invalid");
                }
            } else {
                visual = map.get(id);
            }
    ...
    public Visual getVisual(int id) {
        final String sql = "SELECT * FROM " + Table.VISUALS + " JOIN " + Table.VISUALS_SOURCES + " ON id = visual WHERE id = ?";
    ...
    public List<Visual> getVisualsForSource(int id) {
        final String sql = "SELECT * FROM " + Table.VISUALS + " JOIN " + Table.VISUALS_SOURCES + " ON id = visual WHERE source = ?";
        final Object[] args = new Object[] { id };
        jdbcTemplate.query(sql, args, new FullVisualRowMapper());
    ...
    public List<Visual> listFullVisuals() {
        final String sql = "SELECT * FROM " + Table.VISUALS + " JOIN " + Table.VISUALS_SOURCES + " ON id = visual";
    ...
    public int saveVisual(Visual visual) {
        ...
        if (id == null) {
            final String sql = "INSERT INTO " + Table.VISUALS + " (title) VALUES (?);";
            final Object[] args = new Object[] { visual.getTitle() };
            jdbcTemplate.update(sql, args);
            id = jdbcTemplate.queryForInt("CALL IDENTITY();");
        } else {
            final String sql = "UPDATE " + Table.VISUALS + " SET title = ? WHERE id = ?";
            final Object[] args = new Object[] { visual.getTitle(), id };
            jdbcTemplate.update(sql, args);
        }
        // update visual-sources relationships
        String sql = "SELECT * FROM " + Table.VISUALS_SOURCES + " WHERE visual = ?";
        List<Integer> sources = jdbcTemplate.query(sql, new Object[] { id }, new VisualSourcesRowMapper());
        for (int source : visual.getSourcesIds()) {
            if (!sources.contains(source)) {
                sql = "INSERT INTO " + Table.VISUALS_SOURCES + " (visual, source) VALUES (?, ?);";
        ...
        for (Integer source : sources) {
            if (!visual.getSourcesIds().contains(source)) {
                sql = "DELETE FROM " + Table.VISUALS_SOURCES + " WHERE visual = ? AND source = ?";
                final Object[] args = new Object[] { id, source };
                jdbcTemplate.update(sql, args);
            }
        }
    ...
    public int saveChartVisual(ChartVisual visual) {
        ...
        final String sql = "UPDATE " + Table.VISUALS + " SET class = ?, type = ?, yaxis = ? WHERE id = ?";
        final Object[] args = new Object[] { "chart", visual.getType(), visual.getYaxis(), id };
        jdbcTemplate.update(sql, args);
        return id;
    }
    ...
    public int saveScatterVisual(ScatterVisual visual) {
        int id = this.saveVisual(visual);
        final String sql = "UPDATE " + Table.VISUALS + " SET class = ?, yaxis = ? WHERE id = ?";
